...
SELECT DISTINCT T.`id` , T.`surname` as `insegnanti` , DEP.`name` as `dipartimento`
FROM `departments` as DEP 
JOIN `degrees` as DEG ON DEP.`id` = DEG.`department_id`
...
SELECT COUNT(ES.`vote`) as `tentativi` , S.`registration_number` as `matricola` , S.`name` as `nome` , S.`surname` as `cognome`
FROM `students` as S
